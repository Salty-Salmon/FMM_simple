...
double Interaction_uniform_field::calc_energy (std::vector<Particle *> &gas){
    double energy = 0;
    for(auto pcl : gas){
...
void Interaction_uniform_field::calc_force (std::vector<Particle *> &gas){
    for(auto pcl : gas){
        pcl->force += strength * pcl->charge;
    }
}

double Interaction_uniform_field::calc (std::vector<Particle *> &gas){
    double energy = 0;
    for(auto pcl : gas){
        energy -= strength * pcl->pos * pcl->charge;
        pcl->force +=  strength * pcl->charge;
